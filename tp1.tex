...
\makeindex % Initializes the index

\renewcommand{\indexname}{Índice} % Rename the index

\let\oldindex\index % Save the original \index command
\renewcommand{\index}[1]{\oldindex{#1|vspace{10pt}}} % Redefine \index to add extra space beteween each entry
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{fiuba.png}
    \label{fig:image}
\end{figure}
...
\index{1@Análisis de la situación}
...
Determinar la cantidad de equipos de Combate del Fuegoy Apoyo
...
\index{4@Modelo!3@Restricciones}
